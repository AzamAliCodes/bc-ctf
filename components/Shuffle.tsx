/**
 * Shuffle — React Bits strip-reveal text animation
...
const Shuffle = ({
    ...
    () => {
      ...
      const removeHover = () => {
        if (hoverHandlerRef.current && ref.current) {
          ref.current.removeEventListener('mouseenter', hoverHandlerRef.current as EventListener);
      ...
      const build = () => {
        ...
        const chars: HTMLElement[] = (splitRef.current.chars || []) as HTMLElement[];
        wrappersRef.current = [];
        ...
        const measureVerticalCell = (node: HTMLElement, lineBoxHeight: number) => {
          ...
          if (metricsContext) {
            ...
            const ascent = (metrics as unknown as { fontBoundingBoxAscent?: number }).fontBoundingBoxAscent ?? 0;
            const descent = (metrics as unknown as { fontBoundingBoxDescent?: number }).fontBoundingBoxDescent ?? 0;
        ...
        chars.forEach(ch => {
          ...
          else if (shuffleDirection === 'down')   { startY = -steps * cellHeight; finalY = 0; }
          else if (shuffleDirection === 'up')     { startY = 0;                   finalY = -steps * cellHeight; }
      ...
      const armHover = () => {
        ...
        ref.current.addEventListener('mouseenter', handler as EventListener);
      };
  ...
  // Use createElement to avoid TSX "children expects never" on dynamic tags
